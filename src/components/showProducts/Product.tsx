...
function Product({ name, price, img, imgName, stock }: ProductPropTypes) {
  ...
  return (
    <div className='relative w-56 p-4 rounded-2xl bg-white shadow-2xl ease-in delay-100 overflow-hidden hover:-translate-y-1 hover:drop-shadow-lg'>
      <div className='flex items-center justify-center rounded-lg border-2 border-[#f7f0f0] bg-white overflow-hidden'>
        <img src={img} alt={imgName} className='block max-w-full h-40' />
      </div>
      <div className='flex items-center h-20 mt-3 p-2 text-xl no-underline'>
        <a href='#'>{name}</a>
      </div>
      <div className='flex items-center justify-between flex-wrap mt-3 p-2 pt-4 border-t-[1px] border-gray-300'>
